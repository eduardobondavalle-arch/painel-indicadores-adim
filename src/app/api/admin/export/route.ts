...
type ResponseRow = { id: string; submitted_manager_name: string; submitted_manager_email: string; reference_month: number; reference_year: number; submitted_at: string; protocol: string; status: string };
...
export async function GET(request: NextRequest) {
  ...
  if (type === "response") {
    ...
    const [responseResult, valuesResult] = await Promise.all([
      supabase.from("responses").select("id,submitted_manager_name,submitted_manager_email,reference_month,reference_year,submitted_at,protocol,status").eq("id", id).maybeSingle(),
    ...
    if (format === "pdf") {
      ...
      autoTable(document, { startY: 40, head: [["Nº", "Indicador", "Valor"]], body: values.map((value) => [value.indicator_number, INDICATOR_BY_KEY[value.indicator_key]?.label ?? value.label, String(current(value))]), styles: { fontSize: 7.5, cellPadding: 2 }, headStyles: { fillColor: [16, 43, 78] }, alternateRowStyles: { fillColor: [248, 246, 241] }, columnStyles: { 0: { cellWidth: 10 }, 2: { cellWidth: 38 } } });
  ...
  const responsesResult = await supabase.from("responses").select("id,submitted_manager_name,submitted_manager_email,reference_month,reference_year,submitted_at,protocol,status").eq("reference_month", month).eq("reference_year", year).in("status", ["submitted", "reopened"]).order("submitted_manager_name");
  ...
  const rows = responses.map((response) => {
    ...
    if (type !== "restricted") for (const metric of calculateMetrics(valueMap)) base[`Calculado — ${metric.label}`] = metric.value ?? "";
    return base;
  });
