...
type ResponseRow = { id: string; manager_id: string; submitted_manager_name: string; submitted_manager_email: string; reference_month: number; reference_year: number; submitted_at: string; protocol: string; status: string; consistency_alert_count: number };
  ...
  return (
    <>
      ...
      {!responses.length ? <EmptyState icon={Files} title="Nenhum envio encontrado" description="Ajuste os filtros ou aguarde o recebimento dos formulários deste período." /> : (
        <Card className="overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-left text-sm"><thead className="border-b bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-3">Gestora</th><th className="px-4 py-3">Referência</th><th className="px-4 py-3">Envio</th><th className="px-4 py-3">Protocolo</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Alertas</th><th className="px-5 py-3 text-right">Ações</th></tr></thead><tbody className="divide-y divide-slate-100">{responses.map((response) => { const status = statusLabels[response.status] ?? statusLabels.submitted; return <tr key={response.id} className="hover:bg-slate-50/70"><td className="px-5 py-4"><p className="font-bold text-[#102b4e]">{response.submitted_manager_name}</p><p className="mt-0.5 text-xs text-slate-500">{response.submitted_manager_email}</p></td><td className="px-4 py-4">{String(response.reference_month).padStart(2, "0")}/{response.reference_year}</td><td className="px-4 py-4"><p>{new Date(response.submitted_at).toLocaleDateString("pt-BR")}</p><p className="text-xs text-slate-400">{new Date(response.submitted_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</p></td><td className="px-4 py-4 font-mono text-xs">{response.protocol}</td><td className="px-4 py-4"><Badge variant={status.variant}>{status.label}</Badge></td><td className="px-4 py-4">{response.consistency_alert_count ? <span className="flex items-center gap-1 font-bold text-amber-700"><AlertTriangle className="size-4" />{response.consistency_alert_count}</span> : <span className="text-emerald-700">Sem alertas</span>}</td><td className="px-5 py-4"><div className="flex justify-end gap-1"><Button asChild variant="ghost" size="sm"><Link href={`/gestao/envios/${response.id}`}><Eye className="size-4" />Visualizar</Link></Button><Button asChild variant="ghost" size="sm"><Link href={`/gestao/comparativo?mes=${response.reference_month}&ano=${response.reference_year}&gestora=${response.manager_id}`}><GitCompareArrows className="size-4" />Comparar</Link></Button>{profile.role === "admin" && <ResponseActions id={response.id} status={response.status} />}</div></td></tr>; })}</tbody></table></div></Card>
      )}
    </>
  );
}
