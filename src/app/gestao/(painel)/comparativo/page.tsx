...
type ResponseRow = { id: string; period_id: string; manager_id: string | null; submitted_manager_name: string; submitted_manager_email: string };
...
export default async function ComparisonPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const responsesResult = await supabase.from("responses").select("id,period_id,manager_id,submitted_manager_name,submitted_manager_email").eq("reference_month", month).eq("reference_year", year).in("status", ["submitted", "reopened"]).order("submitted_manager_name");
  ...
  const data = responses.map((response) => ({ id: response.id, managerId: response.manager_id, name: response.submitted_manager_name, email: response.submitted_manager_email, performance: performance.results.get(response.id)!, values: Object.fromEntries(values.filter((value) => value.response_id === response.id).map((value) => [value.indicator_key, value.value_type === "text" ? value.value_text ?? "" : Number(value.value_numeric ?? 0)])) }));
